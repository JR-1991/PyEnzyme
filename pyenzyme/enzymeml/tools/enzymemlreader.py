...
class EnzymeMLReader(object):
    ...
    def __getCreators(self, model):
        ...
        return [
            Creator(
                creator.getFamilyName(), creator.getGivenName(), creator.getEmail()
            )
    ...
    def __getReactions(self, model, enzmldoc):
        ...
        for reac in reaction_list:
            ...
            for child in list(root)[0]:
                ...
                if "conditions" in child.tag:
                    ...
                    for cond in child:
                        ...
                        if 'temperature' in cond.tag:
                            temperature=float(cond.attrib["value"])
                            tempunit=cond.attrib["unit"]
            ...
            educts = [
                ...
                if species_ref.getSpecies() in reaction_replicates
                else (
                    species_ref.getSpecies(),
                    species_ref.getStoichiometry(),
                    species_ref.getConstant(),
            ...
            modifiers = [
                ...
                if species_ref.getSpecies() in reaction_replicates
                else (species_ref.getSpecies(), 1.0, False, list(), list())
    ...
    def __getReplicates(self, reaction_list):
        ...
        columns = [
            f'time/{child.attrib["unit"]}'
            if child.attrib["type"] == "time"
            else f'{child.attrib["replica"]}/{child.attrib["species"]}/{child.attrib["type"]}'
        ...
        data.set_index(f"time/{time_unit}", inplace=True)
        ...
        for format_ in listOfFormats:
            for child in format_:
                ...
                if child.attrib["type"] != "time":
                    ...
                    repl.setData(
                        data[
                            f'{child.attrib["replica"]}/{child.attrib["species"]}/{child.attrib["type"]}'
                        ]
                    )
