...
class EnzymeMLWriter(object):
    ...
    def toFile(self, enzmldoc, path):
        ...
        self.path = f'{path}/{enzmldoc.getName()}'
        ...
        # Write to EnzymeML
        writer = SBMLWriter()
        writer.writeSBMLToFile(doc, f'{self.path}/experiment.xml')

        # Write to OMEX
        self.__createArchive(enzmldoc, doc)

        os.remove(f'{self.path}/experiment.xml')
        os.remove(f'{self.path}/data/data.csv')
        os.rmdir(f'{self.path}/data')
    ...
    def __createArchive(self, enzmldoc, doc):
        ...
        try:
            os.remove(f'{self.path}/{out_file}')
        except FileNotFoundError:
            pass

        archive.writeToFile(f'{self.path}/{out_file}')
    ...
    def __addReactants(self, model, enzmldoc):
        ...
        for key, reactant in enzmldoc.getReactantDict().items():
            ...
            if append_bool: species.appendAnnotation(reactant_annot)
    ...
    def __addReactions(self, model, enzmldoc, csv=True):
        ...
        if not list(enzmldoc.getReactionDict().items()):
        ...
        if csv:
            ...
            df.to_csv(f'{self.path}/data/data.csv', index=False, header=False)
